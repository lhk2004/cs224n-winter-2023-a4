...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded)  # should be in the shape of (src_len, b, e)
        X = X.permute(1, 2, 0)                           # now in the shape of (b, e, src_len)
        # feed X into the convolutional layer
        X = self.post_embed_cnn(X)
        # revert X back to its original shape
        X = X.permute(2, 0, 1)
        # apply the "pack_padded_sequence" function to X
        X = pack_padded_sequence(X, source_lengths)
        # feed the packed X into Bi-LSTM encoder
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
        # apply the "pad_packed_sequence" function to enc_hiddens
        enc_hiddens, lens = pad_packed_sequence(enc_hiddens)
        # permute the enc_hiddens from the shape of (src_len, b, h * 2) to (b, src_len, h * 2)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        #compute dec_init_state
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # apply the attention projection layer to enc_hiddens
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        # target sentences embeddings
        Y = self.model_embeddings.target(target_padded)
        for t, Y_t in enumerate(torch.split(Y, 1)):
            Y_t = torch.squeeze(Y_t, 0)   # from shape (1, b, e) to (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # dec_hidden: (b, h) ---unsqueeze---> (b, h, 1), then batch multiply enc_hiddens_proj (b, src_len, h) by dec_hidden (b, h, 1), get (b, src_len, 1), finally squeeze into (b, src_len)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, dim = 2)), dim = 2)

        ### END YOUR CODE

        # Set e_t to -inf where enc_masks has 1
        # meaning that we do not pay any attention to the pad tokens when translating the source sentence
        ...
        # alpha_t: (b, src_len) ---unsqueeze---> (b, 1, src_len), then batch matrix multiplied by enc_hiddens (b, src_len, 2h), finally get (b, 1, 2h), and squeezed into (b, 2h)
